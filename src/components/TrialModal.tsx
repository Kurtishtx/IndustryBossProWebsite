...
export default function TrialModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  useEffect(() => {
    if (step !== 'done') return;
    const t = setInterval(() => setCountdown(c => { if (c <= 1) { clearInterval(t); window.location.href = APP_URL; return 0; } return c - 1; }), 1000);
  ...
  async function submitPass(e: React.FormEvent) {
    ...
    try {
      ...
      if (uid) {
        ...
        await sb.auth.updateUser({ data: { product: 'fieldbosspro' } });
        await sb.from('user_profiles').insert({ id: uid, first_name: form.first, last_name: form.last, trial_ends_at: trialEnds, plan: 'trial', product: 'fieldbosspro' });
        await sb.from('company_info').insert({ user_id: uid, company_name: form.company });
      }
